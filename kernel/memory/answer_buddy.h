...
void _buddy_return_page(page_t* page) {
    ...
    // Suggested: 4 LoCs
    page_t* bd=_buddy_get_buddy_page(page);
    if(bd->orders==page->orders&&(bd->flags&BD_PAGE_FREE)){
        bd->flags=0;
        list_del(&bd->list_node);
        if(_buddy_get_page_idx(bd)< _buddy_get_page_idx(page))
            page=bd;
        page->orders++;
...
void _buddy_split_page(page_t *original_page, uint64 target_order){
    ...
    while(original_page->orders>target_order){
        ...
        _buddy_return_page(_buddy_get_buddy_page(original_page));
    }
}
...
page_t *_buddy_alloc_page(uint64 order){
    // Suggested: 13 LoCs
    uint64 min_free_order=order;
    while(!bd_lists[min_free_order].nr_free&&min_free_order<bd_max_size)++min_free_order;
    if(min_free_order==bd_max_size)return NULL;
    page_t* ret;
    if(min_free_order>order){
        ret=(page_t*)bd_lists[min_free_order].list_head.next;
        _buddy_split_page(ret,order);
    }
    else {
        ret = (page_t *) bd_lists[min_free_order].list_head.next;
        _buddy_get_specific_page(ret);
    }
    return ret;
}
